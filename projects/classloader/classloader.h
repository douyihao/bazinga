...
namespace bzg
{
	...
	public:
			...
			/// for the library to become unloaded.
		{
			...
			typename LibraryMap::iterator it = map_.find(path);
			if (it == map_.end())
			{
				LibraryInfo li;
				li.pLibrary = 0;
				...
				try
				{
					...
					std::string bzgbuildManifestSymbol("pocBuildManifest");
			...
			/// class, or a null pointer if the class is not known.
		{
			...
			for (typename LibraryMap::const_iterator it = map_.begin(); it != map_.end(); ++it)
			{
			...
			/// library, or a null pointer if the library has not been loaded.
		{
			...
			typename LibraryMap::const_iterator it = map_.find(path);
			if (it != map_.end())
